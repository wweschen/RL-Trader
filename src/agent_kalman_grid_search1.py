...
class KSearchAgent(BaseAgent):

    def __init__(self, config, environment):
        super(KSearchAgent, self).__init__(config,environment)
    ...
    def QFunc(self, s,c):
        ...
        print('in:', insig, 'out:', outsig, self.env.position, c, self.time_since, self.sell_out_escape_count,
              self.buy_out_escape_count, q)
                    ...
                    for n_sell_out  in range(min_out_SAX_length,n_out+1): #count of sell out signal character c
                        for n_sell_in in range(min_in_SAX_length,n_in+1):  #count of sell in signal character c
                            for n_buy_out  in range(min_out_SAX_length,n_out+1): #count of buy out signal character a
                                ...
                                c=[n_in,n_out,n_buy_in,n_sell_out,n_sell_in,n_buy_out]
                                #c=[2,6,2,4,2,6]
                                s = self.get_state(self.env.data)

                                action_probs = self.policy(s, c)
                                ...
                                for self.i in tqdm(range(391)):
                                    ...
                                    action_probs = self.policy(s_prime,c)
                                    ...
                                    if self.env.terminal:
                                        ...
                                        self.log_search_result(self.env.today, c, self.account_profit_loss)

                                        self.log_trade(''.join(str(i) for i in c), self.env.today, None, 0,0,0, self.account_profit_loss)
