...
class Agent(BaseModel):
  ...
  def train(self,eventSource):
    ...
    self.theta = np.zeros([19,3],dtype=np.float32)
    self.theta_hat = np.zeros([19, 3], dtype=np.float32)
    self.features =  np.zeros([19], dtype=np.float32)
    self.features_sample = np.zeros([19], dtype=np.float32)
    self.zetha = np.zeros([19,3],dtype=np.float32)
    ...
    s  = np.zeros([19], dtype=np.float32).tolist()
    ...
    for self.step in tqdm(range(start_step, self.max_step), ncols=70, initial=start_step):
      ...
      # print('Next:',self.next_state,'=',best_next_action )
      # td_target = reward + discount_factor * self.Q[self.next_state][best_next_action]
      # td_delta = td_target - self.Q[self.state][self.action]
      # self.Q[self.state][self.action] += alpha * td_delta
      ...
      if terminal:
        ...
        action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
        while action not in self.actions_allowed:
          action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
        num_game += 1
