...
class ChartWidget(FigureCanvas):
    ...
    def  __init__(self,symbol, filepath, width, height, parent,app=None, colorup='g', colordown='r'):
    ...
    def update_chart(self,i=0):
        ...
        self.targets1 = self.axLearning.scatter(range(30), self.target_data1, s=15, c="b", alpha=0.9, marker="o")

        data =self.target_data1+self.target_data1*(0.5-np.random.random(30))*.2

        self.predhistory = self.axLearning.scatter(range(30),  data, s=15, c="k", marker="x")
        ...
        self.targets = self.axForecast.scatter(range(30), self.target_data,s=15,c="k",   marker="x")
        #self.axForecast.set_xticks(np.arange(0, 30, 5))
        #self.axForecast.set_xticks(np.arange(0, 30, 1), minor=True)
