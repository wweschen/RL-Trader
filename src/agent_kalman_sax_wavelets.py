...
class KSWAgent(BaseAgent):

    def __init__(self, config, environment):
        super(KSWAgent, self).__init__(config,environment)
    ...
    def train(self, steps, eventSource):
        ...
        for self.i in tqdm(range(self.config.train_steps)):
            ...
            fifteen_minute = timedelta(minutes=30)
            ...
            if self.env.terminal:
                ...
                self.log_trade_summary(self.env.today, 'wavelets', self.account_profit_loss-self.previous_profit_loss)

                self.log_trade('wavelets', self.env.today, None, 0, 0, 0, self.account_profit_loss-self.previous_profit_loss,'')
